...
class RCPSpider(CrawlSpider):
    ...
    def parse(self, response):
        print 'Start scraping election data....'
        try:
...
#            for RCP, this traverses to the innerHTML of the 1st tables cells
            ...
            v_name = \
                hxs.select("/html/body/div[@id='container']/div[@id='alpha-container']/div[@id='alpha']/div[@id='table-1']/table/tbody/tr/td/a/text()"
                           ).extract()
            if not v_name:
...
#                might get an array of tables returned, specify index to try again
                ...
                v_name = \
                    hxs.select("/html/body/div[@id='container']/div[@id='alpha-container']/div[@id='alpha']/div[@id='table-1']/table[0]/tbody/tr/td/a/text()"
                               ).extract()

#            On RealClearPolitics, all tables have a header of the poll, grab that name

            title = \
                hxs.select("/html/body/div[@id='container']/div[@id='alpha-container']/div[@id='alpha']/div[@id='page_title']/div/text()"
                           ).extract()
